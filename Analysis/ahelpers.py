...
def dejectify_fnl(fnl, method='naor1'):
    """Quick-and-dirty filtering of ejecta field from SPHERAL output fnl."""
    ...
    # Remove ejecta and repack to fnl
    data = data[ind]
    primary = pack_fnl(data)
...
global fnl_header_dejecta
fnl_header_dejecta = """\
###############################################################################
 This file contains the bound nodes from an impact simulation. Included are
 field variables as well as some diagnostic data and node meta data. Original
 simulation had M={:0.4g} kg in {} nodes.
 This file was generated by identifying the largest gravitationally bound
 fragment, with M_lb={:0.4g} kg, moving to that fragment's center of mass
 frame, and then removing all nodes not belonging to the largest fragment.
 This file should contain {} data lines, one per SPH node identified as
 belonging to the largest fragment. Line order is not significant. The columns
 contain field values in whatever units where used in the simulation, hopefully
 MKS.
 Columns are:
 | id | eos_id | x | y | z | vx | vy | vz | m | rho | p | T | U | h | R_ini |
