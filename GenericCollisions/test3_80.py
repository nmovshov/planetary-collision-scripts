#! /home/benke/sph/spheral/bin/python
#-------------------------------------------------------------------------------
# Test of two solid, strength-regime spheres into each other.
# Use 80 nodes across the target.
# 
# This script launches a basic strength regime collision using Tillotsen EOS.
#
...
jobName = 'test3'
jobDesc = "Simple collision of solid, strength regime spheres."
print '\n', jobName.upper(), '-', jobDesc.upper()

# Target parameters
rTarget = 37.5e3             # Target radius (m)
...
# Impactor parameters
rImpactor = 6.93e3            # Impactor radius (m)
...
vImpact = 5000               # Impact velocity (m/s)
...
# BLE: ADJUST nxTarget TO CHANGE THE RESOLUTION?
# Times, simulation control, and output
nxTarget = 80                # Nodes across diameter of target (run "resolution"), was 20
...
# BLE: ADJUST nPerh TO CHANGE THE RESOLUTION TOO?
# Node list parameters
nPerh = 2.01                 # Nominal number of nodes per smoothing scale, was 2.01
...
impactor = makeSolidNodeList('impactor', eosImpactor, strengthTarget,
                             nPerh = nPerh, 
                             xmin = -universeEdge*Vector.one, # (probably unnecessary)
                             xmax =  universeEdge*Vector.one, # (probably unnecessary)
                             hmin = hmin,
...
# Build some history objects to follow the time evolution of stuff.
    strainHistory = AverageStrain(damageModelTarget,
                                  os.path.join(jobDir, "strainhistory.txt"))

# Create the time integrator and attach the physics packages to it.
integrator = CheapSynchronousRK2Integrator(db)
#integrator.appendPhysicsPackage(gravity)      # BLE REMOVED GRAVITY
