...
#-------------------------------------------------------------------------------
# Run two fluid, self-gravitating, uniform density spheres into each other.
# 
# This script launches a basic gravity regime collision. Two spherical, fluid
# objects of arbitrary size and (initially) uniform density collide with some 
# specified velocity and impact parameter. In spheral terms, the target and 
# impactor are of type FluidNodeList, and the nodes are all assigned a constant
# density to being with. The only physics packages attached to the integrator are
# the hydro package and the gravity package. Although the target and impactor may
# select a solid material equation of state, they do not possess any elastic 
# strength, and no damage model is attached. 
#
# This script can be used to model collision of small bodies such as asteroids or
# small moons. For larger bodies, the uniform density assumption is not valid, and
# the bodies need to be pre-relaxed to hydrostatic equilibrium before colliding.
#
...
# Gravity parameters
softLength = 1.0/nxTarget    # Fraction of target radius as softening length
